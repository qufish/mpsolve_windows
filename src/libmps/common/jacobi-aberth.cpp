...
{
    __mps_fjacobi_aberth_step_data* data = (__mps_fjacobi_aberth_step_data*)data_ptr;
    ...
    mps_fjacobi_aberth_step(mps_context* ctx, mps_polynomial* p, int* nit)
{
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            ...
            * the function call system has been optimized out. */
            if (ctx->pool->n > 1)
            {
                mps_new_obj(__mps_fjacobi_aberth_step_data, data, sizeof(__mps_fjacobi_aberth_step_data));
            ...
            {
                __mps_fjacobi_aberth_step_data data = {
            ctx, p, ctx->approx_root[i], &corrections[i]
                };
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            cplx_sub_eq(ctx->approx_root[i]->fvalue, corrections[i]);
            ctx->approx_root[i]->frad += cplx_mod(corrections[i]);
            again = true;
        }
    }
    ...
    mps_faberth_packet(mps_context* ctx, mps_polynomial* p, mps_boolean just_regenerated)
{
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (!(ctx->approx_root[i]->approximated || MPS_ROOT_STATUS_IS_COMPUTED(ctx->approx_root[i]->status)))
...
{
    __mps_djacobi_aberth_step_data* data = (__mps_djacobi_aberth_step_data*)data_ptr;
    cdpe_t abcorr;
    ...
    mps_djacobi_aberth_step(mps_context* ctx, mps_polynomial* p, int* nit)
{
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            mps_new_obj(__mps_djacobi_aberth_step_data, data, sizeof(__mps_djacobi_aberth_step_data));
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            ...
            cdpe_sub_eq(ctx->approx_root[i]->dvalue, daberth_corrections[i]);
            cdpe_mod(correction_module, daberth_corrections[i]);
            rdpe_add_eq(ctx->approx_root[i]->drad, correction_module);
        }
    }
...
{
    __mps_mjacobi_aberth_step_data* data = (__mps_mjacobi_aberth_step_data*)data_ptr;
    mpc_t corr, abcorr;
    ...
    mps_mjacobi_aberth_step(mps_context* ctx, mps_polynomial* p, int* nit)
{
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            mps_new_obj(__mps_mjacobi_aberth_step_data, data, sizeof(__mps_mjacobi_aberth_step_data));
    ...
    for (i = 0; i < ctx->n; i++)
    {
        if (ctx->approx_root[i]->again)
        {
            ...
            mpc_sub_eq(ctx->approx_root[i]->mvalue, maberth_corrections[i]);
