...
* system, is located in input-output.cpp */
...
copy = mps_newv(char, (int)(ptr - exponent_start) + 1);
...
/* This an over estimate of the length of the _real_ token that we
    ...
    * over-estimate. */
char* allocated_line = mps_strdup(orig_line);
char* copy = mps_newv(char, 2 * (int)strlen(orig_line) + 5);
char* copy_ptr = copy;
...
{
char* input_copy = mps_strdup(input);
