...
mps_approximation*
    mps_approximation_new(mps_context* ctx)
{
    mps_new_obj(mps_approximation, appr, sizeof(mps_approximation));

    mpc_init2(appr->mvalue, ctx->mpwp);
    appr->again = true;
...
{
    mps_approximation* new_approx = mps_approximation_new(ctx);

    mpc_set_prec(new_approx->mvalue, mpc_get_prec(original->mvalue));
    mpc_set(new_approx->mvalue, original->mvalue);
    rdpe_set(new_approx->drad, original->drad);
    cdpe_set(new_approx->dvalue, original->dvalue);
    cplx_set(new_approx->fvalue, original->fvalue);
    new_approx->frad = original->frad;
    new_approx->wp = original->wp;
    new_approx->status = original->status;
    new_approx->attrs = original->attrs;
    new_approx->inclusion = original->inclusion;
    return new_approx;
}
    ...
    mps_approximation_set_mvalue(mps_context* ctx, mps_approximation* approximation, const mpc_t value)
{
    ...
    if ((int)mpc_get_prec(value) > approximation->wp)
    {
