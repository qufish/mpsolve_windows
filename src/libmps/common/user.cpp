...
*/
void
    mps_fnewton_usr(mps_context* ctx, mps_polynomial* poly, mps_approximation* root, cplx_t corr)
{
    cplx_t p, pp, pt, tmp;
    double ap, ax, eps;
    int i, m;
    ...
    m = (int)(log(ctx->n + 1.0) / LOG2);
    if ((1 << m) <= ctx->n)
        m++;
    eps = (DBL_EPSILON * 4.0) * ctx->n;
    ax = cplx_mod(x);
    ...
    *rad = ctx->n * (cplx_mod(p) + 3 * ap * eps) / cplx_mod(pp);
}
...
*/
MPS_PRIVATE void
    mps_dnewton_usr(mps_context* ctx, mps_polynomial* poly, mps_approximation* root, cdpe_t corr)
{
    cdpe_t p, pp, pt, tmp;
    ...
    rdpe_mul_eq_d(eps, (double)4 * ctx->n);
    cdpe_mod(ax, x);
    ...
    rdpe_mul_eq_d(root->drad, (double)ctx->n);
    cdpe_mod(temp, pp);
...
*/
MPS_PRIVATE void
    mps_mnewton_usr(mps_context* ctx, mps_polynomial* poly, mps_approximation* root, mpc_t corr, long int wp)
{
    int i, m;
    rdpe_t ap, ax, eps, temp, apeps, atmp;
    cdpe_t ctmp;
    ...
    mpc_init2(pp, ctx->mpwp);
    mpc_init2(pt, ctx->mpwp);
    mpc_init2(tmp, ctx->mpwp);

    m = (int)(log(ctx->n + 1.0) / LOG2);
    if ((1 << m) <= ctx->n)
        m++;
    rdpe_set(eps, ctx->mp_epsilon);
    rdpe_mul_eq_d(eps, (double)4 * ctx->n);
