...
MPS_PRIVATE void
    mps_raise_parsing_error(mps_context* ctx, mps_input_buffer* buffer,
        const char* token,
        const char* message, ...)
{
    if (!token)
    {
        mps_error(ctx, message);
        return;
    }

    char* output = (char*)mps_malloc(sizeof(char) * ((int)strlen(token) + 256));
    sprintf(output, "Parsing error on line %ld near the token: %s", buffer->line_number, token);

    mps_error(ctx, output, message);
    mps_free(output);
}
...
MPS_PRIVATE mps_boolean
    mps_is_option(mps_context* ctx, const char* option_string1,
...
MPS_PRIVATE mps_input_option
    mps_parse_option_line(mps_context* ctx, char* line, size_t length)
{
    char* first_comment;
    char* option;
    char* c_ptr;
    ...
    *(c_ptr + 1) = '\0';
    ...
    {
        MPS_DEBUG(ctx, "Parsed option: %s", option);
    }
    ...
    if (mps_is_option(ctx, option, "dense"))
        input_option.flag = MPS_FLAG_DENSE;
    if (mps_is_option(ctx, option, "sparse"))
        input_option.flag = MPS_FLAG_SPARSE;

    /* Options on types */
    if (mps_is_option(ctx, option, "integer"))
        input_option.flag = MPS_FLAG_INTEGER;
    if (mps_is_option(ctx, option, "real"))
        input_option.flag = MPS_FLAG_REAL;
    if (mps_is_option(ctx, option, "complex"))
        input_option.flag = MPS_FLAG_COMPLEX;
    if (mps_is_option(ctx, option, "rational"))
        input_option.flag = MPS_FLAG_RATIONAL;
    if (mps_is_option(ctx, option, "floatingpoint"))
        input_option.flag = MPS_FLAG_FP;

    /* Options on the input type */
    if (mps_is_option(ctx, option, "secular"))
        input_option.flag = MPS_FLAG_SECULAR;
    if (mps_is_option(ctx, option, "monomial"))
        input_option.flag = MPS_FLAG_MONOMIAL;
    if (mps_is_option(ctx, option, "chebyshev"))
    ...
    * return. */
    ...
    if (equal_position == NULL)
    {
        ...
        {
            mps_error(ctx, "Unrecognized option: %s", option);
        }
        return input_option;
    }
    else
    {
        ...
        * equal sign and anything after it */
        c_ptr = option;
        option = (char*)mps_malloc(sizeof(char) * ((int)strlen(option) + 1));
        strcpy(option, c_ptr);
        *strchr(option, '=') = '\0';
    }

    if (mps_is_option(ctx, option, "degree"))
        input_option.flag = MPS_KEY_DEGREE;
    else if (mps_is_option(ctx, option, "precision"))
...
mps_polynomial*
    mps_parse_file(mps_context* ctx, const char* path)
{
    FILE* handle = fopen(path, "r");

    if (!handle)
    {
        mps_error(ctx, "Error while opening file: %s", path);
        return NULL;
    }
    else
    {
        mps_polynomial* poly = mps_parse_stream(ctx, handle);
        fclose(handle);
        return poly;
    }
}
...
MPS_PRIVATE mps_polynomial*
    mps_parse_abstract_stream(mps_context* ctx, mps_abstract_input_stream* stream)
{
    ...
    * their omission */
    ...
    while (parsing_options)
    {
        ...
        if (line == NULL || strchr(line, ';') == NULL)
        {
            if (ctx->debug_level & MPS_DEBUG_IO)
            {
                MPS_DEBUG(ctx, "Finished parsing options");
            }
            ...
            if (first_pass)
            {
                ...
                * to parse it that way */
                MPS_DEBUG_WITH_INFO(ctx, "This is not a MPSolve 3 pol file, so trying with 2.x format");
                poly = MPS_POLYNOMIAL(mps_monomial_poly_read_from_stream_v2(ctx, buffer));
        ...
        else
        {
            first_pass = false;
            input_option =
                mps_parse_option_line(ctx, line, strlen(line));
            ...
            {
                ctx->n = atoi(input_option.value);
                if (ctx->n <= 0)
                {
                    mps_error(ctx, "Degree must be a positive integer");
                    mps_input_buffer_free(buffer);
                    return NULL;
                }
            }
            ...
            {
                input_precision = atoi(input_option.value) * (int)LOG2_10;
                if (input_precision <= 0)
                {
                    mps_error(ctx, "Precision must be a positive integer");
                    mps_input_buffer_free(buffer);
                    return NULL;
                }
            }
    ...
    else if (ctx->debug_level & MPS_DEBUG_IO)
    {
        MPS_DEBUG(ctx, "Degree: %d", ctx->n);
    }
    ...
    case MPS_REPRESENTATION_SECULAR:
        if (ctx->debug_level & MPS_DEBUG_IO)
        {
            MPS_DEBUG(ctx, "Parsing secular equation from stream");
        }
        poly = MPS_POLYNOMIAL(mps_secular_equation_read_from_stream(ctx, buffer, structure, density, input_precision));
        break;

    case MPS_REPRESENTATION_CHEBYSHEV:
        if (ctx->debug_level & MPS_DEBUG_IO)
            MPS_DEBUG(ctx, "Parsing mps_chebyshev_poly from stream");
        poly = MPS_POLYNOMIAL(mps_chebyshev_poly_read_from_stream(ctx, buffer, structure, density, input_precision));
        break;

    case MPS_REPRESENTATION_MONOMIAL:
    default:
        if (ctx->debug_level & MPS_DEBUG_IO)
            MPS_DEBUG(ctx, "Parsing mps_monomial_poly from stream");
        poly = MPS_POLYNOMIAL(mps_monomial_poly_read_from_stream(ctx, buffer, structure, density, input_precision));
        break;
    }
    ...
    if (poly)
    {
        ...
        mps_polynomial_set_input_prec(ctx, MPS_POLYNOMIAL(poly), input_precision);
    }
...
mps_polynomial*
    mps_parse_stream(mps_context* ctx, FILE* input_stream)
{
    if (!input_stream)
        input_stream = ctx->instr;

    mps_skip_comments(input_stream);

    mps_file_input_stream* stream = mps_file_input_stream_new(input_stream);
    mps_polynomial* p = mps_parse_abstract_stream(ctx, (mps_abstract_input_stream*)stream);
...
mps_polynomial*
    mps_parse_string(mps_context* ctx, const char* c_string)
{
    char* input_copy = mps_strdup(c_string);

    mps_memory_file_stream* stream = mps_memory_file_stream_new(input_copy);
    mps_polynomial* poly = mps_parse_abstract_stream(ctx, (mps_abstract_input_stream*)stream);
