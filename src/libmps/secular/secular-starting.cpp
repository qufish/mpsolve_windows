...
*/
void
    mps_secular_fstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations)
{
    ...
    for (i = 0; i < n; i++)
    {
        if (!MPS_ROOT_STATUS_IS_COMPUTED(approximations[i]->status))
        {
            ...
            {
                MPS_DEBUG_CPLX(ctx, approximations[i]->fvalue, "ctx->froot[%d]", i);
            }
        }
    }
}
...
*/
void
    mps_secular_dstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    for (l = 0; l < MPS_POLYNOMIAL(sec)->degree; l++)
    {
        if (!MPS_ROOT_STATUS_IS_COMPUTED(approximations[l]->status))
        {
            ...
            if (ctx->debug_level & MPS_DEBUG_APPROXIMATIONS)
            {
                MPS_DEBUG_CDPE(ctx, approximations[l]->dvalue, "ctx->droot[%d]", l);
            }
        }
    }
}
...
*/
void
    mps_secular_mstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    for (l = 0; l < MPS_POLYNOMIAL(sec)->degree; l++)
    {
        if (!MPS_ROOT_STATUS_IS_COMPUTED(approximations[l]->status))
        {
            ...
            rdpe_mul_eq(bmod, ctx->mp_epsilon);
