...
*/
int
    mps_secular_fparallel_sum(mps_context* ctx, mps_approximation* root, int n,
        cplx_t* afpc, cplx_t* bfpc,
        cplx_t pol, cplx_t fp, cplx_t sumb, double* asum)
{
    ...
    {
        int i = n / 2, k;
        if ((k = mps_secular_fparallel_sum(ctx, root, i, afpc, bfpc, pol, fp, sumb, asum)) >= 0)
        {
            return k;
        }
        if ((k = mps_secular_fparallel_sum(ctx, root, n - i, afpc + i, bfpc + i, pol, fp, sumb, asum)) >= 0)
        {
            return i + k;
        }
...
void
    mps_secular_fnewton(mps_context* ctx, mps_polynomial* p, mps_approximation* root, cplx_t corr)
{
    int i;
    ...
    if ((i = mps_secular_fparallel_sum(ctx, root, MPS_POLYNOMIAL(sec)->degree, sec->afpc,
        sec->bfpc, pol,
        fp, sumb, &asum)) >= 0)
    {
        int k;
    ...
    if ((asum_on_apol + 1) * KAPPA * DBL_EPSILON > 1)
    {
        if (ctx->debug_level & MPS_DEBUG_PACKETS)
            MPS_DEBUG(ctx, "Setting again to false on root for root neighbourhood");
        root->again = false;
    }
    else if (acorr < MPS_SQRT2 * ax * DBL_EPSILON)
    {
        if (ctx->debug_level & MPS_DEBUG_PACKETS)
            MPS_DEBUG(ctx, "Setting again to false on root for small Newton correction");
    ...
    {
        double new_rad = acorr * ctx->n * (1 + KAPPA * DBL_EPSILON * asum_on_apol) +
            ax * DBL_EPSILON * 4.0;
...
*/
int
    mps_secular_dparallel_sum(mps_context* ctx, mps_approximation* root, int n, cdpe_t* adpc, cdpe_t* bdpc,
        cdpe_t pol, cdpe_t fp, cdpe_t sumb, rdpe_t asum)
{
    ...
    {
        int i = n / 2, k;
        if ((k = mps_secular_dparallel_sum(ctx, root, i, adpc, bdpc, pol, fp, sumb, asum)) >= 0)
        {
            return k;
        }
        if ((k = mps_secular_dparallel_sum(ctx, root, n - i, adpc + i, bdpc + i, pol, fp, sumb, asum)) >= 0)
        {
            return i + k;
        }
...
void
    mps_secular_dnewton(mps_context* ctx, mps_polynomial* p, mps_approximation* root, cdpe_t corr)
{
    int i;
    ...
    if ((i = mps_secular_dparallel_sum(ctx, root, MPS_POLYNOMIAL(sec)->degree, sec->adpc, sec->bdpc,
...
*/
int
    mps_secular_mparallel_sum(mps_context* ctx, mps_approximation* root, int n, mpc_t* ampc, mpc_t* bmpc,
        mpc_t pol, mpc_t fp, mpc_t sumb, rdpe_t asum)
{
    ...
    {
        int i = n / 2, k;
        if ((k = mps_secular_mparallel_sum(ctx, root, i, ampc, bmpc, pol, fp, sumb, asum)) >= 0)
        {
            return k;
        }
        if ((k = mps_secular_mparallel_sum(ctx, root, n - i, ampc + i, bmpc + i, pol, fp, sumb, asum)) >= 0)
        {
            return i + k;
        }
...
void
    mps_secular_mnewton(mps_context* ctx, mps_polynomial* p, mps_approximation* root, mpc_t corr, long int wp)
{
    int i;
    ...
    if ((i = mps_secular_mparallel_sum(ctx, root, MPS_POLYNOMIAL(sec)->degree, sec->ampc,
        sec->bmpc, pol,
        fp, sumb, asum)) >= 0)
    {
        int k;
        ...
        * In that case we have to correct the estimate. */
        if (p->prec > 0 && p->prec < (int)mpc_get_prec(pol))
        {
    ...
    if (root->again || mpc_eq_zero(corr))
    {
        ...
        rdpe_mul_d(new_rad, acorr, ctx->n);
