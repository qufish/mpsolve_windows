...
mps_secular_equation*
    mps_secular_equation_read_from_stream(mps_context* ctx,
        ...
        long int precision)
{
    ...
    * what is supported by the standard floating point arithmetic */
    sec = mps_secular_equation_new_raw(ctx, ctx->n);
    MPS_POLYNOMIAL(sec)->degree = ctx->n;
    ...
    if (MPS_STRUCTURE_IS_FP(structure))
    {
        for (i = 0; i < ctx->n; i++)
        {
            ...
            if (!token || (mpf_set_str(mpc_Re(sec->initial_ampc[i]), token, 10) != 0))
            {
                ...
                mps_polynomial_free(ctx, MPS_POLYNOMIAL(sec));
                sec = NULL;
                goto cleanup;
            }
            mps_free(token);
    ...
    */
        ...
        MPS_STRUCTURE_IS_INTEGER(structure))
    {
        for (i = 0; i < ctx->n; i++)
        {
            ...
            {
                MPS_DEBUG(ctx, "Error reading the coefficients a[%d] of the secular equation (real part)", i);
                mps_raise_parsing_error(ctx, buffer, token,
                    "Error reading some coefficients of the secular equation.\n"
                    "Please check your input file");
            ...
            if (MPS_STRUCTURE_IS_COMPLEX(structure))
            {
                ...
                {
                    MPS_DEBUG(ctx, "Error reading the coefficients a[%d] of the secular equation (imaginary part)", i);
                    mps_raise_parsing_error(ctx, buffer, token,
                        "Error reading some coefficients of the secular equation."
                        "Please check your input file");
                    mps_free(token);
            ...
            {
                MPS_DEBUG(ctx, "Error reading the coefficients b[%d] of the secular equation (real part)", i);
                mps_raise_parsing_error(ctx, buffer, token,
                    "Error reading some coefficients of the secular equation."
                    "Please check your input file");
                mps_free(token);
            ...
            if (MPS_STRUCTURE_IS_COMPLEX(structure))
            {
                ...
                {
                    MPS_DEBUG(ctx, "Error reading the coefficients b[%d] of the secular equation (imaginary part)", i);
                    mps_raise_parsing_error(ctx, buffer, token,
                        "Error reading some coefficients of the secular equation."
                        "Please check your input file");
                    mps_free(token);
