...
*/
void
    mps_secular_dump(mps_context* ctx, mps_secular_equation* sec)
{
    int i;

    MPS_DEBUG(ctx, "Dumping secular equation:");
    ...
    case mp_phase:
        for (i = 0; i < ctx->n; ++i)
        {
            MPS_DEBUG_MPC(ctx, 20, sec->ampc[i], "sec->ampc[%d]", i);
            MPS_DEBUG_MPC(ctx, 20, sec->bmpc[i], "sec->bmpc[%d]", i);
        }
        break;
...
void
    mps_secular_restart(mps_context* ctx)
{
    ...
    for (i = 0; i < ctx->n; i++)
    {
        mpc_get_cplx(ctx->approx_root[i]->fvalue, ctx->approx_root[i]->mvalue);
        mpc_get_cdpe(ctx->approx_root[i]->dvalue, ctx->approx_root[i]->mvalue);
    }
}
...
*
* Please note the <code>ctx->n</code> and <code>ctx->deg</code>
...
*/
void
    mps_secular_deflate(mps_context* ctx, mps_secular_equation* sec)
{
    int i, j, k;
    ...
    * DPE input */
    if (MPS_STRUCTURE_IS_FP(MPS_POLYNOMIAL(sec)->structure))
    {
        ...
        * correctly right now */
        MPS_DEBUG_WITH_INFO(ctx, "Floating point deflation still has some rough edges, so it's disabled");
        return;
    }
    ...
    * values */
    ...
    MPS_DEBUG(ctx, "Secular equation deflated to degree %d", MPS_POLYNOMIAL(sec)->degree);
}
...
mps_secular_equation*
    mps_secular_equation_new_raw(mps_context* ctx, unsigned long int n)
{
    unsigned long int i;

    mps_new_obj(mps_secular_equation, sec, sizeof(mps_secular_equation));

    mps_polynomial_init(ctx, MPS_POLYNOMIAL(sec));
    ...
    p->freeres = mps_secular_equation_free;
    ...
    mps_secular_equation_double_buffer* db = &sec->db;
    ...
    mpc_vinit2(sec->db.ampc1, n, ctx->mpwp);
    mpc_vinit2(sec->db.bmpc1, n, ctx->mpwp);
    mpc_vinit2(sec->db.ampc2, n, ctx->mpwp);
    mpc_vinit2(sec->db.bmpc2, n, ctx->mpwp);
    mpc_vinit2(sec->initial_ampc, n, ctx->mpwp);
    mpc_vinit2(sec->initial_bmpc, n, ctx->mpwp);
    ...
    mps_new_defined_array_obj(mps_mutex_t, sec->ampc_mutex, sizeof(mps_mutex_t), MPS_POLYNOMIAL(sec)->degree);
    mps_new_defined_array_obj(mps_mutex_t, sec->bmpc_mutex, sizeof(mps_mutex_t), MPS_POLYNOMIAL(sec)->degree);

    for (i = 0; i < n; i++)
    {
        mps_mutex_init(sec->ampc_mutex[i]);
        mps_mutex_init(sec->bmpc_mutex[i]);
    }

    mps_mutex_init(sec->precision_mutex);
...
*/
mps_secular_equation*
    mps_secular_equation_new(mps_context* ctx, cplx_t* afpc, cplx_t* bfpc,
        unsigned long int n)
{
    unsigned long int i;

    /* Allocate the space for the new struct */
    mps_secular_equation* sec = mps_secular_equation_new_raw(ctx, n);
    ...
    for (i = 0; i < (unsigned long int)MPS_POLYNOMIAL(sec)->degree; i++)
    {
    ...
    mps_secular_equation_set_coefficient_f(mps_context* ctx, mps_secular_equation* p, int i, mpc_t a, mpc_t b)
{
    ...
    if (mpc_get_prec(b) > (unsigned long int)wp)
        wp = mpc_get_prec(b);
    ...
    mps_secular_equation_free(mps_context* ctx, mps_polynomial* p)
{
    ...
    mpc_vclear(sec->db.ampc1, MPS_POLYNOMIAL(sec)->degree);
    mpc_vclear(sec->db.bmpc1, MPS_POLYNOMIAL(sec)->degree);
    mpc_vclear(sec->db.ampc2, MPS_POLYNOMIAL(sec)->degree);
    mpc_vclear(sec->db.bmpc2, MPS_POLYNOMIAL(sec)->degree);

    mpc_vfree(sec->db.ampc1);
    mpc_vfree(sec->db.ampc2);
    mpc_vfree(sec->db.bmpc1);
    mpc_vfree(sec->db.bmpc2);

    rdpe_vfree(sec->aadpc);
    rdpe_vfree(sec->abdpc);
    double_vfree(sec->aafpc);
    double_vfree(sec->abfpc);

    /* And old coefficients */
    mpc_vclear(sec->initial_ampc, MPS_POLYNOMIAL(sec)->degree);
    mpc_vclear(sec->initial_bmpc, MPS_POLYNOMIAL(sec)->degree);
    mpq_vclear(sec->initial_ampqrc, MPS_POLYNOMIAL(sec)->degree);
    mpq_vclear(sec->initial_bmpqrc, MPS_POLYNOMIAL(sec)->degree);
    mpq_vclear(sec->initial_ampqic, MPS_POLYNOMIAL(sec)->degree);
    mpq_vclear(sec->initial_bmpqic, MPS_POLYNOMIAL(sec)->degree);
    mpc_vfree(sec->initial_ampc);
    mpc_vfree(sec->initial_bmpc);
    mpq_vfree(sec->initial_ampqrc);
    mpq_vfree(sec->initial_bmpqrc);
    mpq_vfree(sec->initial_ampqic);
    mpq_vfree(sec->initial_bmpqic);

    /* Mutexes */
    for (i = 0; i < MPS_POLYNOMIAL(sec)->degree; i++)
    {
        mps_mutex_destroy(sec->ampc_mutex[i]);
        mps_mutex_destroy(sec->bmpc_mutex[i]);
    }
...
*/
void
    mps_secular_evaluate(mps_context* ctx, cplx_t x, cplx_t sec_ev)
{
    cplx_t ctmp;
    int i;
    mps_secular_equation* sec = (mps_secular_equation*)ctx->secular_equation;
...
*/
void
    mps_secular_check_data(mps_context* ctx, char* which_case)
{
    /* While we can't found a good criterion to check
    * the possibility to start in pure floating point we
    * use the DPE version. */
    *which_case = (ctx->input_config->starting_phase == float_phase) ? 'f' : 'd';
}
...
*/
long int
    mps_secular_raise_coefficient_precision(mps_context* ctx, mps_polynomial* p, long int wp)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    mps_mutex_lock(sec->precision_mutex);

    if (wp < (int)mpc_get_prec(sec->ampc[0]))
    {
        mps_mutex_unlock(sec->precision_mutex);
    ...
    {
        MPS_DEBUG_WITH_INFO(ctx, "Precision of the coefficients is now at %ld bits", wp);
    }
...
*/
void
    mps_secular_raise_root_precision(mps_context* ctx, int wp)
{
    MPS_DEBUG_THIS_CALL(ctx);
    int i;
    ...
    for (i = 0; i < ctx->n; i++)
    {
        mpc_set_prec(ctx->approx_root[i]->mvalue, wp);
    }
}
...
*/
void
    mps_secular_raise_precision(mps_context* ctx, int wp)
{
    MPS_DEBUG_THIS_CALL(ctx);

    int i;

    mps_secular_raise_coefficient_precision(ctx, MPS_POLYNOMIAL(ctx->secular_equation), wp);
    mps_secular_raise_root_precision(ctx, wp);

    ctx->mpwp = wp;
    rdpe_set_2dl(ctx->mp_epsilon, 1.0, -wp);
...
*/
void
    mps_secular_switch_phase(mps_context* ctx, mps_phase phase)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    if (phase == mp_phase)
    {
        ...
        * to the multiprecision phase */
        ...
        {
            MPS_DEBUG(ctx, "Dumping current approximations before starting MP");
            mps_dump(ctx);
        }

        mps_secular_raise_precision(ctx, MPS_SECULAR_STARTING_MP_PRECISION);
        switch (ctx->lastphase)
        {
        case float_phase:
        ...
        /* Set epsilon */
        rdpe_set_2dl(ctx->mp_epsilon, 1.0, -ctx->mpwp + 1);
    }
    else
    {
...
*/
void
    mps_secular_set_radii(mps_context* ctx)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    mpc_t mtmp;
    mpc_init2(mtmp, mps_context_get_data_prec_max(ctx));

    if (ctx->lastphase == mp_phase)
        rdpe_set(rad_eps, ctx->mp_epsilon);
    else
        rdpe_set_d(rad_eps, DBL_EPSILON);

    rdpe_mul_eq_d(rad_eps, ctx->n * 4);
    ...
    for (i = 0; i < ctx->n; i++)
    {
        ...
        rdpe_mul_eq_d(rad, (double)ctx->n);

        rdpe_set(drad[i], rad);

        mpc_rmod(rtmp, ctx->approx_root[i]->mvalue);
        if (ctx->lastphase == mp_phase)
            rdpe_mul_eq(rtmp, ctx->mp_epsilon);
        else
    ...
    case float_phase:
    {
        ...
        {
            rdpe_set_d(ctx->approx_root[i]->drad, ctx->approx_root[i]->frad);
            mpc_set_d(ctx->approx_root[i]->mvalue, cplx_Re(ctx->approx_root[i]->fvalue),
                cplx_Im(ctx->approx_root[i]->fvalue));
        }

        mps_mcluster(ctx, drad, 2 * ctx->n);
        mps_fmodify(ctx, false);

        for (i = 0; i < ctx->n; i++)
        {
            ctx->approx_root[i]->frad = rdpe_get_d(ctx->approx_root[i]->drad);
            if (ctx->approx_root[i]->frad == 0.0)
                ctx->approx_root[i]->frad += cplx_mod(ctx->approx_root[i]->fvalue) * DBL_EPSILON;
        }
    }
    break;
