...
* <code>ctx->approx_root[i]->wp</code> with the given precision rounded to the
...
MPS_PRIVATE long int
    mps_secular_ga_update_root_wp(mps_context* ctx, int i, long int wp, mpc_t* bmpc)
{
    ...
    ctx->approx_root[i]->wp = ((wp - 1) / min_prec + 1) * min_prec;

    mps_mutex_lock(ctx->data_prec_max.value_mutex);

    if (ctx->data_prec_max.value < ctx->approx_root[i]->wp)
        ctx->data_prec_max.value = ctx->approx_root[i]->wp;
    mps_mutex_unlock(ctx->data_prec_max.value_mutex);

    if (ctx->debug_level & MPS_DEBUG_MEMORY)
        MPS_DEBUG(ctx, "Setting wp for root %d to %ld bits", i, ctx->approx_root[i]->wp);

    if (mpc_get_prec(sec->ampc[i]) < (unsigned long int)ctx->approx_root[i]->wp)
        mpc_set_prec(sec->ampc[i], ctx->approx_root[i]->wp);

    mps_polynomial_raise_data(ctx, p, ctx->approx_root[i]->wp);

    return ctx->approx_root[i]->wp;
}
...
*/
static mps_boolean*
    mps_secular_ga_find_changed_roots(mps_context* ctx, cdpe_t* old_b, mpc_t* old_mb)
{
    ...
    mps_secular_equation* sec = ctx->secular_equation;
    mps_boolean* root_changed = mps_boolean_valloc(ctx->n);
    ...
    for (i = 0; i < ctx->n; i++)
    {
        ...
        * regeneration strategy, so we use the general strategy all the time. */
        if (ctx->just_raised_precision || true)
        {
            root_changed[i] = true;
            continue;
        }
        ...
        * CDPE. */
        ...
        if ((ctx->debug_level & MPS_DEBUG_REGENERATION) && !root_changed[i])
            MPS_DEBUG(ctx, "b_%d hasn't changed, so p(b_%d) will not be recomputed", i, i);
    ...
    if (changed_roots != 0)
        MPS_DEBUG(ctx, "%d of %d approximations are different from last regeneration", changed_roots, ctx->n);
...
{
    __mps_secular_ga_regenerate_coefficients_monomial_data* data = (__mps_secular_ga_regenerate_coefficients_monomial_data*)data_ptr;
    ...
    * this to ctx->mpwp; */
    ...
    /* mps_secular_raise_coefficient_precision (ctx, coeff_wp); */
    ...
    case dpe_phase:
        ...
        * at ctx->mp_epsilon */
    case mp_phase:
        rdpe_set(root_epsilon, ctx->mp_epsilon);
        break;
    ...
    mpc_set_si(lc, -1, 0);
    mps_polynomial_get_leading_coefficient(ctx, p, ctmp);
    mpc_div_eq(lc, ctmp);
    ...
    */
    if (root_changed[i])
    {
        ...
        * disturb other threads at work. */
        mpc_init2(tx, ctx->approx_root[i]->wp);

        /* Give a sensible minimum bound to the necessary precision */
        ctx->approx_root[i]->wp = (long int)(MAX(ctx->mpwp + log2(ctx->n), ctx->approx_root[i]->wp));
        mps_secular_ga_update_root_wp(ctx, i, ctx->approx_root[i]->wp, bmpc);

        mpc_set(tx, bmpc[i]);
        mps_polynomial_meval(ctx, p, tx, sec->ampc[i], relative_error);
        ...
        {
            MPS_DEBUG_MPC(ctx, mpc_get_prec(sec->ampc[i]), sec->ampc[i], "p(b_%d)", i);
        }

        if (ctx->debug_level & MPS_DEBUG_REGENERATION)
        {
            MPS_DEBUG_RDPE(ctx, relative_error, "Relative_error on p(b_%d) evaluation", i);
        }

        if (rdpe_gt(relative_error, root_epsilon))
        {
            long int required_precision = 1 + ctx->approx_root[i]->wp + (rdpe_Esp(relative_error) - rdpe_Esp(root_epsilon));
            if (ctx->debug_level & MPS_DEBUG_MEMORY)
                MPS_DEBUG(ctx, "Setting precision to %ld", required_precision);
        ...
        if (mpc_get_prec(mprod_b) < (unsigned long int)ctx->approx_root[i]->wp)
        {
            mpc_set_prec(mprod_b, ctx->approx_root[i]->wp);
            mpc_set_prec(lc, ctx->approx_root[i]->wp);
            mpc_set_si(lc, -1, 0);
            mps_polynomial_get_leading_coefficient(ctx, p, ctmp);
            mpc_div_eq(lc, ctmp);
        }

        mps_mutex_lock(sec->bmpc_mutex[i]);
        mpc_set(my_b, bmpc[i]);
        mps_mutex_unlock(sec->bmpc_mutex[i]);
        ...
        for (j = 0; j < ctx->n; ++j)
        {
            ...
            * that we need more precision in the roots */
            ...
            {
                MPS_DEBUG(ctx, "Regeneration of the coefficients failed because sec->bdpc[%d] == sec->bdpc[%d]", i, j);
                MPS_DEBUG_MPC(ctx, ctx->mpwp / LOG2_10 + 3, my_b, "b_%d", i);
                MPS_DEBUG_MPC(ctx, ctx->mpwp / LOG2_10 + 3, bmpc[j], "b_%d", j);
                success = false;
        ...
        * a_i, as requested. */
        ...
        if (ctx->debug_level & MPS_DEBUG_REGENERATION)
        {
            MPS_DEBUG_MPC(ctx, ctx->mpwp, sec->ampc[i], "a_%d", i);
            MPS_DEBUG_MPC(ctx, ctx->mpwp, sec->bmpc[i], "b_%d", i);
        }
...
* in <code>ctx->monomial_poly</code>.
*
* @param ctx The <code>mps_context</code> of the computation
...
*/
static mps_boolean
    mps_secular_ga_regenerate_coefficients_monomial(mps_context* ctx, cdpe_t* old_b, mpc_t* old_mb, mps_boolean* root_changed)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    __mps_secular_ga_regenerate_coefficients_monomial_data* data =
        mps_newv(__mps_secular_ga_regenerate_coefficients_monomial_data, ctx->n);

    MPS_DEBUG(ctx, "Regenerating coefficients from monomial input");
    ...
    for (i = ctx->n - 1; i >= 0; i--)
    {
        ...
        mps_thread_pool_assign(ctx, ctx->pool, __mps_secular_ga_regenerate_coefficients_monomial_worker,
            &data[i]);
    }
...
* the rigth thing based on <code>ctx->lastphase</code>, where
* <code>ctx</code> is a pointer to the <code>mps_context</code>
...
* where <code>ctx->lastphase == mp_phase</code> to determine if the roots have changed. Otherwise
* it must be set to NULL.
*/
mps_boolean
    mps_secular_ga_regenerate_coefficients_mp(mps_context* ctx, cdpe_t* old_b, mpc_t* old_mb)
{
    ...
    * polynomial only in that approximations. */
    mps_boolean* root_changed = mps_secular_ga_find_changed_roots(ctx, old_b, old_mb);

    /* Regenerate the coefficients of the secular equation starting from the monomial input */
    success = mps_secular_ga_regenerate_coefficients_monomial(ctx, old_b, old_mb, root_changed);

    if (!success)
        MPS_DEBUG(ctx, "Regeneration of the coefficients failed");
    ...
    mps_secular_ga_separate_approximations(mps_context* ctx)
{
    ...
    * current working precision. */
    ...
    for (i = 0; i < ctx->n; i++)
    {
        current_approximations[i] = ctx->approx_root[i];
    ...
    mps_free(current_approximations);
...
MPS_PRIVATE mps_boolean
    mps_secular_ga_regenerate_coefficients(mps_context* ctx)
{
    MPS_DEBUG_THIS_CALL(ctx);

    ctx->operation = MPS_OPERATION_REGENERATION;

    mps_secular_equation* sec;
    int i;
    mps_boolean successful_regeneration = true;

    sec = (mps_secular_equation*)ctx->secular_equation;

    MPS_DEBUG_WITH_INFO(ctx, "Regenerating coefficients");
    ...
    mps_secular_ga_separate_approximations(ctx);
    ...
    mps_regeneration_driver* driver = ctx->regeneration_driver;
    ...
    case float_phase:
        driver->update_fsecular_equation(ctx, ctx->active_poly, ctx->approx_root, sec);
        break;

        /* If this is the DPE phase regenerate DPE coefficients */
    case dpe_phase:
        driver->update_dsecular_equation(ctx, ctx->active_poly, ctx->approx_root, sec);
        break;

    case mp_phase:
        driver->update_msecular_equation(ctx, ctx->active_poly, ctx->approx_root, sec);
        break;

    default:
        break;
    }                           /* End of switch (ctx->lastphase) */

    mps_secular_set_radii(ctx);

    /* Sum execution time to the total counter */
#ifndef DISABLE_DEBUG
    ctx->regeneration_time += mps_stop_timer(my_clock);
#endif

    if (successful_regeneration)
    {
        MPS_DEBUG(ctx, "Setting again to true");
        for (i = 0; i < ctx->n; i++)
