    ...
    _mps_fradii_worker(void* data_ptr)
{
    ...
    mps_polynomial_get_leading_coefficient(ctx, p, lc);

    /* Compute the value of the polynomial in this point */
    mps_polynomial_feval(ctx, p, ctx->approx_root[i]->fvalue, pol, &relative_error);
    ...
    new_rad = cplx_mod(pol) + relative_error + cplx_mod(ctx->approx_root[i]->fvalue) * 4.0 * DBL_EPSILON;
    new_rad *= ctx->n;
    ...
    for (j = 0; j < ctx->n; j++)
    {
        ...
        cplx_sub(diff, ctx->approx_root[i]->fvalue, ctx->approx_root[j]->fvalue);
    ...
    * computation using DPE arithmetic.*/
    ...
    fradii[i] = p->degree * new_rad *
        ...
        cplx_mod(ctx->approx_root[i]->fvalue) * DBL_EPSILON * 2.0
        + DBL_MIN;
...
*/
MPS_PRIVATE void
    mps_fradii(mps_context* ctx, mps_polynomial* p, double* fradii)
{
    ...
    {
        for (i = 0; i < ctx->n; i++)
            fradii[i] = ctx->approx_root[i]->frad;
        return;
    }

    for (i = 0; i < ctx->n; i++)
    {
        mps_new_obj(_mps_fradii_worker_data, data, sizeof(_mps_fradii_worker_data));
        ...
        mps_thread_pool_assign(ctx, ctx->pool, _mps_fradii_worker, data);
    }
...
*/
MPS_PRIVATE void
    mps_dradii(mps_context* ctx, mps_polynomial* p, rdpe_t* dradii)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    {
        for (i = 0; i < ctx->n; i++)
            rdpe_set(dradii[i], ctx->approx_root[i]->drad);
        return;
    }
    ...
    {
        cdpe_t diff;
        mps_polynomial_deval(ctx, p, ctx->approx_root[i]->dvalue, pol, relative_error);

        cdpe_mod(new_rad, pol);
        rdpe_add_eq(new_rad, relative_error);
        cdpe_mod(rtmp, ctx->approx_root[i]->dvalue);
        rdpe_mul_eq_d(rtmp, 4.0 * DBL_EPSILON);
        rdpe_add_eq(new_rad, rtmp);
        rdpe_mul_eq_d(new_rad, ctx->n);
        ...
        for (j = 0; j < ctx->n; j++)
        {
            ...
            cdpe_sub(diff, ctx->approx_root[i]->dvalue, ctx->approx_root[j]->dvalue);
...
*/
MPS_PRIVATE void
    mps_mradii(mps_context* ctx, mps_polynomial* p, rdpe_t* dradii)
{
    MPS_DEBUG_THIS_CALL(ctx);
    ...
    mpc_init2(pol, ctx->mpwp);
    mpc_init2(mdiff, ctx->mpwp);

    for (i = 0; i < ctx->n; i++)
    {
        mps_polynomial_meval(ctx, p, ctx->approx_root[i]->mvalue, pol, relative_error);

        mpc_get_cdpe(cpol, pol);
        cdpe_mod(new_rad, cpol);
        rdpe_add_eq(new_rad, relative_error);
        mpc_get_cdpe(cdtmp, ctx->approx_root[i]->mvalue);
        cdpe_mod(rtmp, cdtmp);
        ...
        for (j = 0; j < ctx->n; j++)
        {
            ...
            mpc_sub(mdiff, ctx->approx_root[i]->mvalue, ctx->approx_root[j]->mvalue);
        ...
        rdpe_mul_eq_d(new_rad, 1 + 2 * ctx->n * ::sqrt(2) * DBL_EPSILON);
        rdpe_mul_eq_d(new_rad, p->degree);

        {
            mpc_t lc;
            mpc_init2(lc, ctx->mpwp);
            mps_polynomial_get_leading_coefficient(ctx, p, lc);
            mpc_rmod(rtmp, lc);
            rdpe_div_eq(new_rad, rtmp);
            mpc_clear(lc);
