...
MPS_PRIVATE void
    mps_debug_cluster_structure(mps_context* ctx)
{
    ...
    for (cluster_item = ctx->clusterization->first_cluster_item; cluster_item != NULL; cluster_item = cluster_item->next_cluster_item)
    {
        ...
        * after. */
        ...
        __MPS_DEBUG(ctx, "Found cluster of %ld roots: ",
            cluster->n);

        for (root = cluster->first_root; root != NULL; root = root->next_root)
        {
            fprintf(ctx->logstr, "%ld ", root->k);
        }
        fprintf(ctx->logstr, "\n");
    }
    ...
    if (isolated_roots)
    {
        __MPS_DEBUG(ctx, "Isolated roots: ");
        for (cluster_item = ctx->clusterization->first_cluster_item; cluster_item != NULL; cluster_item = cluster_item->next_cluster_item)
        {
            cluster = cluster_item->cluster;
            if (cluster->n == 1)
                fprintf(ctx->logstr, "%ld ", cluster->first_root->k);
        }
        fprintf(ctx->logstr, "\n");
    }
}
