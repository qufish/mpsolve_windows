    ...
    mps_input_buffer_new(mps_abstract_input_stream* stream)
{
    ...
    buf->history = (char**)mps_malloc(sizeof(char*) * (int)buf->history_size);
    ...
    mps_input_buffer_free(mps_input_buffer* buffer)
{
    ...
    if (buffer->line)
        mps_free(buffer->line);

    for (i = 0; i < buffer->history_size; ++i)
    {
        if (buffer->history[i])
            mps_free(buffer->history[i]);
    }

    mps_free(buffer->history);
    mps_free(buffer);
}
    ...
    mps_input_buffer_readline(mps_input_buffer* buf)
{
    ...
    if (buf->line != NULL)
    {
        new_pos = (buf->last - 1 + (int)buf->history_size) % buf->history_size;
    ...
    * results in an empty line, read a new one. */
    do {
        read_chars = (int)mps_abstract_input_stream_readline(buf->stream, &buf->line, &length);
        ...
        if (buf->line && read_chars > 0)
        {
            ...
            {
                *comment = '\0';
                read_chars = (int)(comment - buf->line);
            }
        }
    ...
    mps_input_buffer_next_token(mps_input_buffer* buf)
{
    ...
    ret = (char*)mps_malloc(sizeof(char) * ((int)token_size + 1));
