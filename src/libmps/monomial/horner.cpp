...
MPS_PRIVATE void
    mps_mhorner_sparse(mps_context* ctx, mps_monomial_poly* p, mpc_t x, mpc_t value);
...
*/
MPS_PRIVATE void
    mps_mhorner(mps_context* ctx, mps_monomial_poly* p, mpc_t x, mpc_t value)
{
    int j;

    if (MPS_DENSITY_IS_SPARSE(ctx->active_poly->density))
    {
        mps_mhorner_sparse(ctx, p, x, value);
    }
    else
    {
        mps_mutex_lock(p->mfpc_mutex[MPS_POLYNOMIAL(p)->degree]);
        mpc_set(value, p->mfpc[MPS_POLYNOMIAL(p)->degree]);
        mps_mutex_unlock(p->mfpc_mutex[MPS_POLYNOMIAL(p)->degree]);

        for (j = MPS_POLYNOMIAL(p)->degree - 1; j >= 0; j--)
        {
            mpc_mul_eq(value, x);

            mps_mutex_lock(p->mfpc_mutex[j]);
            mpc_add_eq(value, p->mfpc[j]);
            mps_mutex_unlock(p->mfpc_mutex[j]);
        }
    }
}
...
* @param wp The working precision to use for the computation. If this value is <code>0</code> then <code>ctx->mpwp</code>
* will be used.
*/
void
    mps_mhorner_with_error2(mps_context* ctx, mps_monomial_poly* p, mpc_t x, mpc_t value, rdpe_t error, long int wp)
{
    int i;
    rdpe_t apol, ax, u;
    cdpe_t cx;

    mps_mutex_lock(p->mfpc_mutex[0]);
    if (mpc_get_prec(p->mfpc[0]) < (unsigned long int)wp)
    {
        mps_mutex_unlock(p->mfpc_mutex[0]);
        mps_monomial_poly_raise_precision(ctx, MPS_POLYNOMIAL(p), wp);
    }
    else
        mps_mutex_unlock(p->mfpc_mutex[0]);

    if (mpc_get_prec(x) < (unsigned long int)wp)
        mpc_set_prec(x, wp);
...
*/
MPS_PRIVATE void
    mps_mhorner_with_error(mps_context* ctx, mps_monomial_poly* p, mpc_t x, mpc_t value, rdpe_t relative_error, long int wp)
{
    int j, my_wp;
    mpc_t ss;
...
*  <code>p[i] == 0</code>.
* @param ctx RDPE pointer in which the result will be stored
...
*/
MPS_PRIVATE void
    mps_mhorner_sparse(mps_context* ctx, mps_monomial_poly* p, mpc_t x,
        mpc_t value)
{
    int m, j, i, i1, i2, q;
    mpc_t tmp, y;
    ...
    * in the input polynomial. */
    ...
    for (i = 0; i < n; i++)
        if (b[i])
        {
            mps_mutex_lock(p->mfpc_mutex[i]);
            mpc_set(mfpc2[i], p->mfpc[i]);
            mps_mutex_unlock(p->mfpc_mutex[i]);
        }
...
*/
void
    mps_dhorner(mps_context* ctx, mps_monomial_poly* p, cdpe_t x, cdpe_t value)
{
    int j;
...
*/
void
    mps_dhorner_with_error(mps_context* ctx, mps_monomial_poly* p, cdpe_t x, cdpe_t value, rdpe_t error)
{
    rdpe_t ax;
    int j;

    mps_dhorner(ctx, p, x, value);
...
*/
void
    mps_fhorner(mps_context* ctx, mps_monomial_poly* p, cplx_t x, cplx_t value)
{
    int j;
...
*/
void
    mps_fhorner_with_error(mps_context* ctx, mps_monomial_poly* p, cplx_t x, cplx_t value, double* error)
{
    int j;
    double ax = cplx_mod(x);

    mps_fhorner(ctx, p, x, value);
