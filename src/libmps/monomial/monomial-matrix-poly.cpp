...
{
    mps_new_obj(mps_monomial_matrix_poly, poly, sizeof(mps_monomial_matrix_poly));
    ...
    MPS_POLYNOMIAL(poly)->freeres = mps_monomial_matrix_poly_free;
        ...
        mpc_t x, mpc_t value, rdpe_t error)
{
    ...
    * in here, as for the case of mps_monomial_poly. */
    if (mpc_get_prec(mpoly->mP[0]) < (int)mpc_get_prec(value))
