...
*/
mps_monomial_poly*
    mps_monomial_poly_new(mps_context* ctx, long int degree)
{
    int i;
    mps_new_obj(mps_monomial_poly, mp, sizeof(mps_monomial_poly));

    mps_polynomial_init(ctx, MPS_POLYNOMIAL(mp));
    ...
    poly->freeres = mps_monomial_poly_free;
    ...
    * the floating point versions. */
    ...
    mpf_vinit2(mp->mfpr, degree + 1, ctx->mpwp);
    mpc_vinit2(mp->db.mfpc1, degree + 1, ctx->mpwp);
    mpc_vinit2(mp->db.mfpc2, degree + 1, ctx->mpwp);
    ...
    mpc_vinit2(mp->mfppc, degree + 1, ctx->mpwp);
    ...
    * exact */
    ...
    mps_mutex_init(mp->regenerating_mutex);

    mps_new_defined_array_obj(mps_mutex_t, mp->mfpc_mutex, sizeof(mps_mutex_t), degree + 1);

    for (i = 0; i <= degree; i++)
        mps_mutex_init(mp->mfpc_mutex[i]);
...
*/
void
    mps_monomial_poly_free(mps_context* ctx, mps_polynomial* p)
{
    ...
    mps_mutex_destroy(mp->regenerating_mutex);
...
*
* @param ctx  The status of the computation.
...
*/
long int
    mps_monomial_poly_raise_precision(mps_context* ctx, mps_polynomial* p, long int prec)
{
    mps_monomial_poly* mp = MPS_MONOMIAL_POLY(p);
    int k;
    mpc_t* raising_mfpc;

    mps_mutex_lock(mp->regenerating_mutex);

    /* For floating point coefficients, it might happen that
    * the coefficients already have higher precision. In that
    * case, we return now. */
    if (prec <= mp->prec ||
        (MPS_STRUCTURE_IS_FP(p->structure) && mpc_get_prec(mp->mfpc[0]) >= (unsigned long int)prec))
    {
        MPS_DEBUG_WITH_INFO(ctx, "Not increasing precision, the coefficients are already at the required accuracy");
        mps_mutex_unlock(mp->regenerating_mutex);
        return mp->prec;
    }
...
long int
    mps_monomial_poly_get_precision(mps_context* ctx, mps_monomial_poly* mp)
{
...
*
* @param ctx The mps_context the will be used for the computation. This shall be passed
...
*/
void
    mps_monomial_poly_set_coefficient_q(mps_context* ctx, mps_monomial_poly* mp, long int i,
...
*
* @param ctx The <code>mps_context</code> associated to this computation.
...
*/
void
    mps_monomial_poly_set_coefficient_d(mps_context* ctx, mps_monomial_poly* mp, long int i,
...
void
    mps_monomial_poly_set_coefficient_int(mps_context* ctx, mps_monomial_poly* mp, long int i,
...
void
    mps_monomial_poly_set_coefficient_f(mps_context* ctx, mps_monomial_poly* p, long int i,
        mpc_t coeff)
{
    /* Updating data_type information */
...
*/
void
    mps_monomial_poly_set_coefficient_s(mps_context* ctx, mps_monomial_poly* p,
        int i, const char* real_coeff,
        const char* imag_coeff)
{
    char* eq_real = mps_utils_build_equivalent_rational_string(ctx, real_coeff);
    char* eq_imag = mps_utils_build_equivalent_rational_string(ctx, imag_coeff);
    ...
    if (eq_real != NULL)
        ...
        mps_warn(ctx, "Invalid input for mps_monomial_set_coefficient_s: %s", real_coeff);

    if (eq_imag != NULL)
        mpq_set_str(tmp_imag, eq_imag, 10);
    else
        mps_warn(ctx, "Invalid input for mps_monomial_set_coefficient_s: %s", imag_coeff);

    mps_monomial_poly_set_coefficient_q(ctx, p, i, tmp_real, tmp_imag);
    ...
    int i, mpq_t real_output, mpq_t imag_output)
{
    ...
    {
        mps_error(ctx, "Cannot extract rational coefficients from a floating point polynomial");
        return;
    }
...
*/
mps_monomial_poly*
    mps_monomial_poly_derive(mps_context* ctx, mps_monomial_poly* p, int k, long int wp)
{
    mps_monomial_poly* d = mps_monomial_poly_new(ctx, MPS_POLYNOMIAL(p)->degree - k);
    int i, j;
    ...
    if (wp != ctx->mpwp)
        mps_monomial_poly_raise_precision(ctx, MPS_POLYNOMIAL(d), wp);
    ...
    case MPS_STRUCTURE_REAL_RATIONAL:
    {
        ...
        for (i = 0; i <= MPS_POLYNOMIAL(d)->degree; i++)
        {
            ...
            mps_monomial_poly_set_coefficient_q(ctx, d, i, coeff_r, coeff_i);
        }
    ...
    default:
    {
        ...
        for (i = 0; i <= MPS_POLYNOMIAL(d)->degree; i++)
            mps_monomial_poly_set_coefficient_f(ctx, d, i, coeffs[i]);
...
void mps_monomial_poly_deflate(mps_context* ctx, mps_polynomial* poly)
{
    ...
    for (int i = 0; i < zero_roots; i++) {
        mps_mutex_destroy(p->mfpc_mutex[poly->degree + i + 1]);
    }
}
