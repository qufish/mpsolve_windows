...
*/
MPS_PRIVATE void
    mps_fnewton(mps_context* ctx, mps_polynomial* poly, mps_approximation* root,
        cplx_t corr)
{
    int i;
...
*/
MPS_PRIVATE void
    mps_dnewton(mps_context* ctx, mps_polynomial* poly, mps_approximation* root,
        cdpe_t corr)
{
    int i;
    ...
    if (cdpe_ne(p, cdpe_zero))
        ...
        {
            if (ctx->DOLOG)
                fprintf(ctx->logstr, "%s", "NULL DERIVATIVE\n");
...
*/
MPS_PRIVATE void
    mps_mnewton(mps_context* ctx, mps_polynomial* poly,
        mps_approximation* root, mpc_t corr, long int wp)
{
    ...
    if (MPS_DENSITY_IS_SPARSE(poly->density))
    {
        ...
        * by one. */
        ...
        mps_polynomial_init(ctx, MPS_POLYNOMIAL(&derivative));

        MPS_POLYNOMIAL(&derivative)->degree = MPS_POLYNOMIAL(mp)->degree - 1;
        derivative.spar = mp->spar + 1;
        derivative.prec = mp->prec;
        derivative.mfpc_mutex = mp->mfpc_mutex + 1;  // advance to the next mutex in the array, so this is a ptr to a mutex not a new mutex
        ...
        mps_polynomial_meval(ctx, MPS_POLYNOMIAL(mp), root->mvalue, p, ap);
        mps_mhorner(ctx, &derivative, root->mvalue, p1);
