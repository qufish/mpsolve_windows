...
*/
mps_monomial_poly*
    mps_monomial_poly_read_from_stream(mps_context* ctx,
        ...
        long int precision)
{
    ...
    * if we are trying to solve the associated secular_equation */
    poly = mps_monomial_poly_new(ctx, ctx->n);
    ...
    if (MPS_DENSITY_IS_DENSE(density))
    {
        if (MPS_STRUCTURE_IS_FP(structure))
        {
            for (i = 0; i < ctx->n + 1; ++i)
            {
                ...
                {
                    mps_raise_parsing_error(ctx, buffer, token, "Error parsing coefficients of the polynomial");
                    mps_free(token); mps_polynomial_free(ctx, MPS_POLYNOMIAL(poly));
                    return NULL;
                }
                mps_free(token);
    ...
    } /* closes if (MPS_INPUT_CONFIG_IS_DENSE (ctx->input_config)) */
    else if (MPS_DENSITY_IS_SPARSE(density))
    {
        ...
        * any coefficient */
        ...
        while ((token = mps_input_buffer_next_token(buffer)))
        {
            ...
            if (!sscanf(token, "%d", &i))
            {
                mps_raise_parsing_error(ctx, buffer, token, "Error while parsing the degree of a monomial");
                mps_free(token); mps_polynomial_free(ctx, MPS_POLYNOMIAL(poly));
                return NULL;
            }
            ...
            if (i < 0 || i > ctx->n)
            {
                mps_raise_parsing_error(ctx, buffer, token, "Degree of coefficient out of bounds");
                mps_free(token); mps_polynomial_free(ctx, MPS_POLYNOMIAL(poly));
                return NULL;
            }
            ...
            if (poly->spar[i])
            {
                mps_raise_parsing_error(ctx, buffer, token, "A monomial of the same degree has been inserted twice");
                mps_free(token); mps_polynomial_free(ctx, MPS_POLYNOMIAL(poly));
                return NULL;
            }
            else
    ...
    } /* closes if (MPS_INPUT_CONFIG_IS_SPARSE (ctx->input_config)) */
...
*/
mps_polynomial*
    mps_monomial_poly_read_from_stream_v2(mps_context* ctx, mps_input_buffer* buffer)
{
    int i;
    ...
    * that should be the case. */
    ...
    {
        mps_error(ctx, "Error parsing the input file");
    ...
    default:
        mps_error(ctx, "Found unsupported data_type in input file");
        goto cleanup;
        break;
    }
    ...
    default:
        mps_error(ctx, "Found unsupported data_structure in input file");
        goto cleanup;
        break;
    }
    ...
    default:
        mps_error(ctx, "Found unsupported data structure in input file");
        goto cleanup;
        break;
    }
    ...
    {
        mps_error(ctx, "Error while reading the input precision of the coefficients");
    ...
    * of the floating point types. */
    ...
    token = mps_input_buffer_next_token(buffer);
    if (!token || !sscanf(token, "%d", &ctx->n))
    {
        mps_error(ctx, "Error reading the degree of the polynomial");
    ...
    * to create custom Newton iteration routines and recomm*/
    ...
    {
        mps_polynomial* user_poly = mps_polynomial_new(ctx);
        ...
        MPS_POLYNOMIAL(user_poly)->degree = ctx->n;
        ...
        else if (MPS_STRUCTURE_IS_RATIONAL(structure))
        {
            ...
            */
            for (i = 0; i <= ctx->n; ++i)
            {
                ...
                {
                    mps_raise_parsing_error(ctx, buffer, token, "Error parsing the numerator of a coefficient");
                ...
                if (!token || (mpq_set_str(qtmp, token, 10)) != 0)
                {
                    mps_raise_parsing_error(ctx, buffer, token, "Error parsing the denominator of a coefficient");
...
/* Copy coefficients back in other places */
    for (i = 0; i <= ctx->n; ++i)
    {
        if (poly->spar[i])
        {
            ...
            {
                MPS_DEBUG_MPC(ctx, 15, poly->mfpc[i], "Coefficient of degree %d", i);
            }
        }
        else
        {
