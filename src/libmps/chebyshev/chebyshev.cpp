...
void mps_chebyshev_poly_free(mps_context* ctx, mps_polynomial* poly);
...
/* This is implemented in chebyshev-evaluation.cpp */
...
{
    mps_new_obj(mps_chebyshev_poly, poly, sizeof(mps_chebyshev_poly));
    ...
    * demand by mps_chebyshev_poly_set_coefficient_q(). */
    ...
    MPS_POLYNOMIAL(poly)->freeres = mps_chebyshev_poly_free;
    ...
    mps_mutex_init(poly->precision_mutex);
    ...
    mps_chebyshev_poly_free(mps_context* ctx, mps_polynomial* poly)
{
    ...
    if (MPS_STRUCTURE_IS_INTEGER(poly->structure) || MPS_STRUCTURE_IS_RATIONAL(poly->structure))
    {
        ...
        mps_free(cpoly->rational_real_coeffs);
        mps_free(cpoly->rational_imag_coeffs);
    }
    ...
    mps_chebyshev_poly_raise_data(mps_context* ctx, mps_polynomial* poly, long int wp)
{
    ...
    mps_mutex_lock(cpoly->precision_mutex);

    /* Check if raising precision is a worth operation, or if we are already
    * to a higher preicison. */
    if (wp < (int)mpc_get_prec(cpoly->mfpc[0]))
    {
        mps_mutex_unlock(cpoly->precision_mutex);
