...
int
    cplx_set_str(cplx_t x, const char* string)
    /* set from string as (re , im) */
{
    return sscanf(string, CPLX_INP_FMT, &cplx_Re(x), &cplx_Im(x)) == 2;
}
...
char*
    cplx_get_str(char* string, const cplx_t x)
    /* output to string as (re , im) */
{
    if (string == NULL && (string = (char*)mps_malloc(DEF_STR_SIZE)) == NULL)
        return NULL;
    sprintf(string, CPLX_OUT_FMT, cplx_Re(x), cplx_Im(x));
    return string;
}
...
int
    rdpe_set_str(rdpe_t e, const char* string)
    /* input from string as mant x exp) */
{
    if (sscanf(string, RDPE_INP_FMT, &rdpe_Mnt(e), &rdpe_Esp(e)) != 2)
        return 0;
...
char*
    rdpe_get_str(char* string, const rdpe_t e)
    /* output to string */
{
    double d;
    long int l;
    ...
    if (string == NULL && (string = (char*)mps_malloc(DEF_STR_SIZE)) == NULL)
        return NULL;
    rdpe_get_dl(&d, &l, e);
    sprintf(string, RDPE_OUT_FMT, d, l);
    return string;
}
...
int
    cdpe_set_str(cdpe_t c, const char* string)
    /* set from string as (re , im) */
{
    if (sscanf
    (string, CDPE_INP_FMT, &rdpe_Mnt(cdpe_Re(c)), &rdpe_Esp(cdpe_Re(c)),
...
char*
    cdpe_get_str(char* string, const cdpe_t c)
    /* output to string as (re , im) */
{
    double dr, di;
    long int lr, li;
    ...
    sprintf(string, CDPE_OUT_FMT, dr, lr, di, li);
    return string;
}
