...
{
    mps_tls_create(key, mps_tls, key_init, tls);
...
static void
    adjust_mps_tls_precision(mps_tls* tls, long int precision_needed)
{
    int i;
...
{
    mps_once(once_key_created, create_key_method);
    mps_tls* tls = (mps_tls*)mps_tls_getspecific(key, key_init);

    /* This means that we have to create a new entry */
    if (tls == NULL)
    {
        tls = create_new_mps_tls(precision_needed);
    }
    else if (tls->precision < precision_needed || precision_needed < .25 * tls->precision)
    {
        adjust_mps_tls_precision(tls, precision_needed);
    }
...
{
    prec = (prec <= 2) ? 53 : prec;
    if ((unsigned long int)mpf_get_prec(mpc_Re(c)) < prec)
    {
...
{
    return (unsigned long int)mpf_get_prec(mpc_Re(c));
}
...
{
    if (mpf_set_str(mpc_Re(c), sr, base) != 0)
        return -1;
...
{
    mpf_t* t = init((long)mpf_get_prec(f));
...
{
    mpf_t* f = init((long)mpf_get_prec(mpc_Re(rc))) + 5;
...
{
    mpf_t* f = init((long)mpf_get_prec(mpc_Re(rc)));
    ...
    mpc_flip(mpc_t rc, mpc_t c)
{
    ...
    mpf_init2(f, (long)mpf_get_prec(mpc_Re(rc)));
    ...
    mpc_mul(mpc_t rc, mpc_t c1, mpc_t c2)
{
    ...
    s1 = init((long)mpf_get_prec(mpc_Re(rc)));
    s2 = s1 + 1;
    s3 = s2 + 1;
    ...
    mpc_div(mpc_t rc, mpc_t c1, mpc_t c2)
{
    ...
    mpc_init2(t, (long)mpf_get_prec(mpc_Re(rc)));
