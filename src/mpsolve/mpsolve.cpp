    ...
    status(int signal)
{
    ...
    fprintf(stderr, "\nOperation running now: %s\n\n", MPS_OPERATION_TO_STRING(ctx->operation));
    ...
    for (i = 0; i < ctx->n; i++)
    {
        ...
        case float_phase:
            ...
            cplx_outln_str(logstr, ctx->root[i]->fvalue);
            break;

        case dpe_phase:
            fprintf(logstr, "  Approximation  %4d = ", i);
            cdpe_outln_str(logstr, ctx->root[i]->dvalue);
            break;

        case mp_phase:
            fprintf(logstr, "  Approximation  %4d = ", i);
            mpc_outln_str(logstr, 10, ctx->mpwp, ctx->root[i]->mvalue);
            break;
        }
    }
    ...
    for (i = 0; i < ctx->n; i++)
    {
        ...
        case mp_phase:
            ...
            rdpe_outln_str(logstr, ctx->root[i]->drad);
            break;
        }
    }
    ...
    for (i = 0; i < ctx->n; i++)
    {
        fprintf(logstr, "  Status  %4d: %-25s  %-15s  %-15s\n", i,
            MPS_ROOT_STATUS_TO_STRING(ctx->root[i]->status),
            MPS_ROOT_ATTRS_TO_STRING(ctx->root[i]->attrs),
            MPS_ROOT_INCLUSION_TO_STRING(ctx->root[i]->inclusion));
    }
    ...
    main(int argc, char** argv)
{
    ...
    * precision. */
    ...
    while ((mps_getopts(&opt, &argc, &argv, MPSOLVE_GETOPT_STRING)))
    {
        ...
        case 'l':
        {
            ...
            if (!logstr)
                mps_error(ctx, "Cannot open selected log file.");
            mps_context_set_log_stream(ctx, logstr);
        }
        break;
        ...
        case 'r':
            mps_context_select_starting_strategy(ctx, MPS_STARTING_STRATEGY_RECURSIVE);
            break;
        ...
        case 'O':
            ...
            if (!opt->optvalue)
            {
                mps_error(ctx, "An argument is needed for option 'O'");
                break;
            }
            ...
            {
            case 'f':
                mps_context_set_output_format(ctx, MPS_OUTPUT_FORMAT_FULL);
                break;
            case 'b':
                mps_context_set_output_format(ctx, MPS_OUTPUT_FORMAT_BARE);
                break;
            case 'g':
                mps_context_set_output_format(ctx, MPS_OUTPUT_FORMAT_GNUPLOT);
                if (*(opt->optvalue + 1) == 'f')
                {
                ...
                break;
            case 'v':
                mps_context_set_output_format(ctx, MPS_OUTPUT_FORMAT_VERBOSE);
                break;
            case 'c':
                mps_context_set_output_format(ctx, MPS_OUTPUT_FORMAT_COMPACT);
                break;
            default:
                mps_error(ctx, "The selected output format is not supported");
                break;
            }
        ...
        case 'S':
            ...
            case 'a':
                ...
                break;
            case 'r':
                ctx->output_config->search_set = MPS_SEARCH_SET_POSITIVE_REAL_PART;
                break;
            case 'l':
                ctx->output_config->search_set = MPS_SEARCH_SET_NEGATIVE_REAL_PART;
                break;
            case 'u':
                ctx->output_config->search_set = MPS_SEARCH_SET_POSITIVE_IMAG_PART;
                break;
            case 'd':
                ctx->output_config->search_set = MPS_SEARCH_SET_NEGATIVE_IMAG_PART;
                break;
            case 'i':
                ctx->output_config->search_set = MPS_SEARCH_SET_UNITARY_DISC;
                break;
            case 'o':
                ctx->output_config->search_set = MPS_SEARCH_SET_UNITARY_DISC_COMPL;
                break;
            case 'R':
            ...
            case 'I':
                ...
                break;
            case 'U':
                ctx->output_config->search_set = MPS_SEARCH_SET_CUSTOM;
                break;
            default:
                mps_error(ctx, "Bad search set switch: ", opt->optvalue,
                    ", use a|r|l|u|d|i|o|R|I|U");
            }
            if (strlen(opt->optvalue) != 1)
                mps_error(ctx, "Bad set: ", opt->optvalue);
            break;
        ...
        case 'M':
            ...
            {
            case '+':
                ctx->output_config->multiplicity = true;
                break;
            case '-':
                ctx->output_config->multiplicity = false;
                break;
            default:
                mps_error(ctx, "Bad multiplicity switch: ", opt->optvalue,
                    ", use +|-");
            }
            if (strlen(opt->optvalue) != 3)
                mps_error(ctx, "Bad multiplicity option: ", opt->optvalue);
            break;
        ...
        case 'D':
            ...
            {
            case 'n':
                mps_context_set_root_properties(ctx, MPS_OUTPUT_PROPERTY_NONE);
                break;
            case 'r':
                mps_context_set_root_properties(ctx, MPS_OUTPUT_PROPERTY_REAL);
                break;
            case 'i':
                mps_context_set_root_properties(ctx, MPS_OUTPUT_PROPERTY_IMAGINARY);
                break;
            case 'b':
                mps_context_set_root_properties(ctx, MPS_OUTPUT_PROPERTY_REAL | MPS_OUTPUT_PROPERTY_IMAGINARY);
                break;
            default:
                mps_error(ctx, "Bad detection switch: ", opt->optvalue,
                    ", use n|r|i|b");
            }
            if (strlen(opt->optvalue) != 1)
                mps_error(ctx, "Bad detection option: ", opt->optvalue);
            break;

            /* I/O streams */
        case 'R':
            mps_context_set_root_stream(ctx, fopen(opt->optvalue, "r"));
            if (ctx->rtstr == NULL)
                mps_error(ctx, "Cannot open roots file: ", opt->optvalue);
            ctx->resume = true;
            break;
        ...
        case 'C':
            ...
            case 'r':
                ...
                break;
            default:
                mps_error(ctx, "Bad check switch: ", opt->optvalue,
                    ", use R|r");
            }
            if (strlen(opt->optvalue) != 1)
                mps_error(ctx, "Bad check option: ", opt->optvalue);
            break;
        ...
        case 'a':
            ...
            case 's':
                ...
                break;
            default:
                mps_error(ctx, "The selected algorithm is not supported");
                break;
            }
            break;
        case 'b':
            mps_context_set_jacobi_iterations(ctx, true);
            break;
        case 'c':
            mps_context_set_crude_approximation_mode(ctx, true);
            break;
        case 'o':
            mps_context_set_output_prec(ctx, (long int)((atoi(opt->optvalue)) * LOG2_10 + 1));
            break;
        case 'i':
            input_precision = (long int)(atoi(opt->optvalue) * LOG2_10);
            break;
        case 'G':
            switch (*opt->optvalue)
            {
            case 'a':
                ...
                break;
            case 'i':
                mps_context_set_output_goal(ctx, MPS_OUTPUT_GOAL_ISOLATE);
                break;
            case 'c':
                mps_context_set_output_goal(ctx, MPS_OUTPUT_GOAL_COUNT);
                break;
            default:
                mps_error(ctx, "The selected goal does not exists");
                break;
            }
            break;

        case 'p':
            inline_poly = _strdup(opt->optvalue);
            break;
        ...
        case 'd':
            ...
            while (*opt->optvalue)
            {
                ...
                case 't':
                    ...
                    break;
                case 'a':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_APPROXIMATIONS);
                    break;
                case 'c':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_CLUSTER);
                    break;
                case 'i':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_IMPROVEMENT);
                    break;
                case 'w':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_TIMINGS);
                    break;
                case 'o':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_IO);
                    break;
                case 'm':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_MEMORY);
                    break;
                case 'f':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_FUNCTION_CALLS);
                    break;
                case 'p':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_PACKETS);
                    break;
                case 'r':
                    mps_context_add_debug_domain(ctx, MPS_DEBUG_REGENERATION);
                    break;
                default:
                    sprintf(output, "Unrecognized debug option: %c", *(opt->optvalue - 1));
                    mps_error(ctx, output);
                    break;
                }
            }
            break;
        ...
        case 's':
            ...
            {
                mps_error(ctx, "You need to specify a valid file with -s");
                break;
            }
            else
            {
                ...
                if (!ctx->rtstr)
                {
                    mps_error(ctx, "Cannot open the given file: %s", opt->optvalue);
                    break;
                }
                else
                {
                    mps_context_select_starting_strategy(ctx, MPS_STARTING_STRATEGY_FILE);
                }
            }
            break;
        ...
        case 'j':
            mps_thread_pool_set_concurrency_limit(ctx, NULL, atoi(opt->optvalue));
            mps_context_set_n_threads(ctx, atoi(opt->optvalue));
            break;
        default:
            usage(ctx, argv[0]);
            break;
    ...
    if (graphic_debug)
    {
        ...
        * and will waste computational time that is likely to bias benchmarks. */
        ...
        mps_iteration_logger_set_mps_context(logger, ctx);
    ...
    if (inline_poly)
    {
        ...
        poly = mps_monomial_yacc_parser(ctx, stream);
    ...
    else
    {
        ...
        if (!infile)
        {
            mps_error(ctx, "Cannot open input file for read, aborting.");
            mps_print_errors(ctx);
            return EXIT_FAILURE;
        }
        ...
        * allocate also a secular equation to be used in regeneration */
        poly = mps_parse_stream(ctx, infile);
    }

    if (!poly)
    {
        mps_error(ctx, "Error while parsing the polynomial, aborting.");
        mps_print_errors(ctx);
        return EXIT_FAILURE;
    }
    else
    ...
    if (input_precision >= 0)
        mps_polynomial_set_input_prec(ctx, poly, input_precision);
    ...
    * hasn't explicitely selected one. */
    ...
    {
        mps_context_select_algorithm(ctx, (MPS_IS_MONOMIAL_POLY(poly) &&
            MPS_DENSITY_IS_SPARSE(poly->density)) ?
            MPS_ALGORITHM_STANDARD_MPSOLVE : MPS_ALGORITHM_SECULAR_GA);
    }

    /* Close the file if it'ctx not stdin */
    if (argc == 2 && !inline_poly)
        fclose(infile);

    /* Select the starting phase according to user input */
    mps_context_set_starting_phase(ctx, phase);

    /* Solve the polynomial */
#if HAVE_GRAPHICAL_DEBUGGER
    if (graphic_debug)
    {
        mps_mpsolve_async(ctx, cleanup_context, NULL);
        gtk_main();
    }
    else
    {
