...
mps_iteration_logger_dispose (GObject *object)
{
  ...
  {
    mps_mutex_destroy (logger->drawing_lock);
...
mps_iteration_logger_init (MpsIterationLogger * logger)
{
  ...
  logger->drawing_lock = mps_malloc(sizeof (mps_mutex_t));
  mps_mutex_init (logger->drawing_lock);
  ...
  mps_mutex_lock(*(logger->drawing_lock));
  logger->approximations = NULL;
  mps_mutex_unlock(*(logger->drawing_lock));
