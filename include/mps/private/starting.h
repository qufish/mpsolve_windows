...
MPS_BEGIN_DECLS
    ...
    /* functions in starting.cpp */
    void mps_fstart(mps_context* ctx, int n, mps_cluster_item* cluster, double clust_rad,
        double g, rdpe_t eps_out, double fap[]);
void mps_dstart(mps_context* ctx, int n, mps_cluster_item* cluster, rdpe_t clust_rad,
    rdpe_t g, rdpe_t eps_out, rdpe_t dap[]);
void mps_mstart(mps_context* ctx, int n, mps_cluster_item* cluster, rdpe_t clust_rad,
    rdpe_t g, rdpe_t dap[], mpc_t gg);
void mps_frestart(mps_context* ctx);
void mps_drestart(mps_context* ctx);
void mps_mrestart(mps_context* ctx);
void mps_fshift(mps_context* ctx, int m, mps_cluster_item* cluster, double clust_rad,
    cplx_t g, rdpe_t eps);
void mps_dshift(mps_context* ctx, int m, mps_cluster_item* cluster, rdpe_t clust_rad,
    cdpe_t g, rdpe_t eps);
void mps_mshift(mps_context* ctx, int m, mps_cluster_item* cluster, rdpe_t clust_rad,
    mpc_t g);

/* functions in recursive-starting.cpp */
...
/* functions in file-starting.cpp */
