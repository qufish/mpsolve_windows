...
*/
#define MPS_USE_BUILTIN_COMPLEX 1
#if MPS_USE_BUILTIN_COMPLEX == 1
...
int cplx_set_str(cplx_t x, const char* string);
...
char* cplx_get_str(char* string, const cplx_t x);
...
/* vector functions */
#define cplx_valloc(N)       (cplx_t *) mps_malloc((N) * sizeof(cplx_t))
void cplx_vinit(cplx_t v[], long size);
/* #define cplx_vclear(V)       mps_free(V) */
/* #define cplx_vclear(V, N)    cplx_vinit(V, N) */
#define cplx_vfree(V)        mps_free(V)
...
int rdpe_set_str(rdpe_t e, const char* string);
...
char* rdpe_get_str(char* string, const rdpe_t e);
...
/* vector functions */
#define rdpe_valloc(N)       (rdpe_t *) mps_malloc((N) * sizeof(rdpe_t))
void rdpe_vinit(rdpe_t v[], long size);
/* #define rdpe_vclear(V)       mps_free(V) */
/* #define rdpe_vclear(V, N)    rdpe_vinit(V, N) */
#define rdpe_vfree(V)        mps_free(V)
...
void cdpe_set_dl(cdpe_t c, double dr, long lr, double di, long li);
void cdpe_set_2dl(cdpe_t c, double dr, long lr,
    double di, long li);
int cdpe_set_str(cdpe_t c, const char* string);
...
char* cdpe_get_str(char* string, const cdpe_t c);
...
/* vector functions */
#define cdpe_valloc(N)       (cdpe_t *) mps_malloc((N) * sizeof(cdpe_t))
void cdpe_vinit(cdpe_t v[], long size);
/* #define cdpe_vclear(V)       mps_free(C) */
/* #define cdpe_vclear(V, N)    cdpe_vinit(V, N) */
#define cdpe_vfree(V)        mps_free(V)
