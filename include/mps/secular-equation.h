...
#define MPS_SECULAR_STARTING_MP_PRECISION 128
    ...
    * in ctx->mpwp for the step of multiprecision coefficient regeneration.
    */
...
struct mps_secular_equation {
    mps_polynomial __base_class__;

    mps_secular_equation_double_buffer db;
    ...
    */
    mps_mutex_t* ampc_mutex;  // this is an array of mutexes
    ...
    */
    mps_mutex_t* bmpc_mutex;  // this is an array of mutexes
...
/* MACROS */
#define mps_secular_equation_from_status(ctx) (mps_secular_equation*)(ctx)->secular_equation

/* Routines in secular-newton.cpp */
...
/* Routines in secular-regeneartion.cpp */
mps_boolean mps_secular_ga_regenerate_coefficients(mps_context* ctx);

/* Routines in secular.cpp */
void mps_secular_deflate(mps_context* ctx, mps_secular_equation* sec);

void mps_secular_check_data(mps_context* ctx, char* which_case);

void mps_secular_restart(mps_context* ctx);

void mps_secular_switch_phase(mps_context* ctx, mps_phase phase);

long int mps_secular_raise_coefficient_precision(mps_context* ctx, mps_polynomial* p, long int wp);

void mps_secular_raise_precision(mps_context* ctx, int wp);

void mps_secular_raise_root_precision(mps_context* ctx, int wp);

/* Routines in secular-starting.cpp */
void mps_secular_fstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations);
void mps_secular_dstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations);
void mps_secular_mstart(mps_context* ctx, mps_secular_equation* sec, mps_approximation** approximations);

/* Routines in secular-iteration.cpp */
int mps_secular_ga_fiterate(mps_context* ctx, int maxit, mps_boolean just_regenerated);

int mps_secular_ga_diterate(mps_context* ctx, int maxit, mps_boolean just_regenerated);

int mps_secular_ga_miterate(mps_context* ctx, int maxit, mps_boolean just_regenerated);

/* Routines in secular-ga.cpp */
mps_boolean mps_secular_ga_check_stop(mps_context* ctx);

void mps_secular_ga_update_coefficients(mps_context* ctx);

/* Interface functions in secular.cpp */
mps_secular_equation* mps_secular_equation_new(mps_context* ctx,
    cplx_t* afpc,
    cplx_t* bfpc,
    unsigned long int n);

mps_secular_equation* mps_secular_equation_new_raw(mps_context* ctx,
    unsigned long int n);
...
void mps_secular_set_radii(mps_context* ctx);
