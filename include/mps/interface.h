...
* to
* @code
* gcc -o myprogram -lmps -lgmp -lm myprogram.cpp.
* @endcode
*
...
* The <code>DPE</code> are have the same precision of the <code>double</code>ctx but allow
...
* them are designed to handle polynomial definition and are implemented in <code>interface.cpp</code>
*
...
/*
	...
	*/
	...
	/* functions in mps_defaults.cpp */
	void mps_set_default_values(mps_context* ctx);

	/* Functions in mps_main.cpp */
	void mps_mpsolve(mps_context* ctx);
	void mps_standard_mpsolve(mps_context* ctx);

	/* functions in mps_interface.cpp */
	void mps_mpsolve_async(mps_context* ctx, mps_callback callback, void* user_data);
