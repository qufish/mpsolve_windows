...
struct mps_context {
    ...
    * be performed with routines in <code>mps_sort.cpp</code>.
    */
    mps_boolean DOSORT;
...
void mps_context_free(mps_context* ctx);

/* Accessor functions (setters) */
void mps_context_abort(mps_context* ctx);
int mps_context_set_poly_d(mps_context* ctx, cplx_t* coeff,
    long unsigned int n);
void mps_context_set_input_poly(mps_context* ctx, mps_polynomial* p);
int mps_context_set_poly_i(mps_context* ctx, int* coeff, long unsigned int n);
void mps_context_select_algorithm(mps_context* ctx, mps_algorithm algorithm);
void mps_context_set_degree(mps_context* ctx, int n);

#ifdef _MPS_PRIVATE
void mps_context_allocate_poly_inplace(mps_context* ctx, int n);
#endif

/* Accessor functions */
long int mps_context_get_data_prec_max(mps_context* ctx);
long int mps_context_get_minimum_precision(mps_context* ctx);
int mps_context_get_degree(mps_context* ctx);
int mps_context_get_roots_d(mps_context* ctx, cplx_t** roots, double** radius);
int mps_context_get_roots_m(mps_context* ctx, mpc_t** roots, rdpe_t** radius);
int mps_context_get_zero_roots(mps_context* ctx);
mps_root_status mps_context_get_root_status(mps_context* ctx, int i);
mps_boolean mps_context_get_over_max(mps_context* ctx);
...
void mps_context_set_input_prec(mps_context* ctx, long int prec);
void mps_context_set_output_prec(mps_context* ctx, long int prec);
void mps_context_set_output_format(mps_context* ctx, mps_output_format format);
void mps_context_set_output_goal(mps_context* ctx, mps_output_goal goal);
void mps_context_set_search_set(mps_context* ctx, mps_search_set set);
void mps_context_set_starting_phase(mps_context* ctx, mps_phase phase);
void mps_context_set_log_stream(mps_context* ctx, FILE* logstr);
void mps_context_set_root_stream(mps_context* ctx, FILE* rtstr);
void mps_context_set_jacobi_iterations(mps_context* ctx, mps_boolean jacobi_iterations);
void mps_context_select_starting_strategy(mps_context* ctx, mps_starting_strategy strategy);
void mps_context_set_avoid_multiprecision(mps_context* ctx, mps_boolean avoid_multiprecision);
void mps_context_set_crude_approximation_mode(mps_context* ctx, mps_boolean crude_approximation_mode);
void mps_context_set_regeneration_driver(mps_context* ctx, mps_regeneration_driver* rd);
...
/* Debugging */
void mps_context_set_debug_level(mps_context* ctx, mps_debug_level level);
void mps_context_add_debug_domain(mps_context* ctx, mps_debug_level level);

/* Get input and output config pointers */
mps_input_configuration* mps_context_get_input_config(mps_context* ctx);
mps_output_configuration* mps_context_get_output_config(mps_context* ctx);
